...
vi.mock("../ToolCallDisplay", () => ({
  ToolCallDisplay: ({ toolInvocation }: any) => (
    <div data-testid="tool-call-display">
      {toolInvocation.toolName}:{toolInvocation.args?.command || "unknown"}
    </div>
  ),
}));
...
test("MessageList renders ToolCallDisplay for tool-invocation parts", () => {
  const messages: Message[] = [
    {
      id: "1",
      role: "assistant",
  ...
  expect(screen.getByTestId("tool-call-display")).toBeDefined();
  expect(screen.getByText("file_manager:delete")).toBeDefined();
});

test("MessageList renders multiple tool-invocation parts", () => {
  const messages: Message[] = [
    {
      id: "1",
      role: "assistant",
  ...
  const toolCalls = screen.getAllByTestId("tool-call-display");
